...
/************** autogenerated part, depending on activated plugins *******/
#include<boost/preprocessor/seq/for_each.hpp>
#include "algorithms/all.h"

typedef int (*scanhash_func_t)(int , uint32_t *, void*, const uint32_t *, uint32_t, unsigned long *, void*);
typedef int (*init_func_t)();
typedef void* (*thread_init_func_t)(int*, void*); // parameter == error code, options
typedef void* (*param_default_func_t)(); 
typedef void* (*param_parse_func_t)(const char *, int *); // parameter == string, error code 


#define GEN_ENUM_ALGO(r,data,algo) BOOST_PP_CAT(ALGO_,algo),
enum algos {
    BOOST_PP_SEQ_FOR_EACH(GEN_ENUM_ALGO,, ALGOS)
};

#define GEN_ALGO_ENTRY(r,data,algo) [BOOST_PP_CAT(ALGO_,algo)] = BOOST_PP_CAT( BOOST_PP_CAT(data,_),algo),

#define GEN_STR_TABLE(var, def) \
static const char * var [] = { \
    BOOST_PP_SEQ_FOR_EACH(GEN_ALGO_ENTRY, def, ALGOS) \
}

GEN_STR_TABLE(algo_names, PLUGIN_NAME);
GEN_STR_TABLE(algo_desc,  PLUGIN_DESC);

#define GEN_FUNC_TABLE(what) \
const BOOST_PP_CAT(what,_func_t) BOOST_PP_CAT(algo_,what) [] = { \
    BOOST_PP_SEQ_FOR_EACH(GEN_ALGO_ENTRY, &what, ALGOS) \
}

GEN_FUNC_TABLE(init);
GEN_FUNC_TABLE(thread_init);
GEN_FUNC_TABLE(scanhash);
GEN_FUNC_TABLE(param_default);
GEN_FUNC_TABLE(param_parse);

#define GEN_HELP_DESC(r,data,algo) "                          " BOOST_PP_CAT(PLUGIN_NAME_, algo) "\t" BOOST_PP_CAT(PLUGIN_DESC_, algo) "\n"
...
static void *opt_algo_param = NULL;
...
Options:\n\
  -a, --algo=ALGO:PARAM specify the algorithm to use, pass optional parameter\n"
BOOST_PP_SEQ_FOR_EACH(GEN_HELP_DESC,,ALGOS)
...
static void *miner_thread(void *userdata)
{
	...
	void *algo_thread_local_data = NULL;
	char s[16];
	int i, error=0;
	...
	algo_thread_local_data = algo_thread_init[opt_algo](&error, opt_algo_param);

	if (error) {
		applog(LOG_ERR, "thread-local data allocation for algorithm %s failed with error %i", 
			   algo_names[opt_algo], error);
	...
	while (1) {
		...
		// TODO: Think about how to make this one more generic for algorithm plugins
		if (have_stratum)
			max64 = LP_SCANTIME;
		else
		...
		if (max64 <= 0) {
			switch (opt_algo) {
			case ALGO_SCRYPT:
				max64 = (size_t)opt_algo_param < 16 ? 0x3ffff : 0x3fffff / (size_t) opt_algo_param;
				break;
			case ALGO_SHA256D:
				max64 = 0x1fffff;
				break;
		...
		rc = algo_scanhash[opt_algo] (thr_id, work.data, algo_thread_local_data, work.target, 
                                      max_nonce, &hashes_done, opt_algo_param); 
	...
	case 'a':
		for (i = 0; i < ARRAY_SIZE(algo_names); i++) {
			...
			if (!strncmp(arg, algo_names[i], v)) {
				if (arg[v] == '\0') {
					opt_algo = i;
					opt_algo_param = algo_param_default[i]();
					break;
				}
				if (arg[v] == ':') {
					param = algo_param_parse[i](arg+v+1, &error);
					if (error == 0) {
						opt_algo = i;
						opt_algo_param = param;
						break;
					} else {
						applog(LOG_ERR, "Error parsing parameters of %s algorithm, error %i.", algo_names[i], error);
						exit(1);
					}
				}
			}
...
int main(int argc, char *argv[])
{
	...
	error = algo_init[opt_algo](); // Call chosen algoritm's init function
	if ( 0 != error ) { 
		applog(LOG_ERR, "Error while initializing %s algorithm, error %i\n",
				algo_names[opt_algo], error);
		exit(1);
	}
