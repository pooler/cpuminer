...
typedef int (*scanhash_func_t)(int , uint32_t *, void*, const uint32_t *, uint32_t, unsigned long *);
typedef int (*init_func_t)();
typedef void* (*thread_init_func_t)(int*); // parameter == error code

//static int scanhash_func_NSCRYPT(int , uint32_t *, void*, const uint32_t *, uint32_t, unsigned long *);

#define COUNT_ALGO(r,op,algo)    +1
#define N_ALGORITHMS (0 + BOOST_PP_SEQ_FOR_EACH(COUNT_ALGO, +, ALGOS))
...
static void *miner_thread(void *userdata)
{
	...
	algo_thread_local_data = algo_thread_init[opt_algo](&error);

	if (error) {
        fprintf(stderr, "algo_thread_init[%s] reported error %i, thus I'll exit.\n",
...
int main(int argc, char *argv[])
{
	...
	if ( 0 != error ) { 
		fprintf(stderr, "ERROR initializing %s algorithm, error %i\n",
				algo_names[opt_algo], error);
		exit(1);
	}
